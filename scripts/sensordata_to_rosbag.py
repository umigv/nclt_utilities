...
import rosbag
import rospy
from std_msgs.msg import Float64, UInt16, Float64MultiArray, \
...
def write_vel(vel_data, utime, bag, num_hits):
    timestamp = rospy.Time.from_sec(utime/1e6)
...
def main(args):
    ...
    while True:
        ...
        if i_gps < len(gps) and \
                (gps[i_gps, 0] < next_utime or next_utime < 0):
            next_packet = "gps"

        if i_gps_rtk < len(gps_rtk) and \
                (gps_rtk[i_gps_rtk, 0] < next_utime or next_utime < 0):
            next_packet = "gps_rtk"

        if i_ms25 < len(ms25) and \
                (ms25[i_ms25, 0] < next_utime or next_utime < 0):
            next_packet = "ms25"

        if i_ms25_euler < len(ms25_euler) and \
                (ms25_euler[i_ms25_euler, 0] < next_utime or next_utime < 0):
        ...
        if next_packet == "done":
            ...
            write_vel(vel_data, utime_vel, bag, num_hits)
            utime_vel, vel_data, num_hits = read_next_vel_packet(f_vel)
